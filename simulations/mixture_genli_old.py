...
n_value = np.array([20,50,100,200,300,400,500,750,1000,1250,1500])
...
def calc_samples_ode(dist0, alpha_t, S=10000):
    # Note: alpha_t[i] = \alpha_{i+1}, alpha_t_bar[i] = \bar{\alpha}_{i+1}
    assert(alpha_t[0] > 0)
    alpha_t_bar = np.cumprod(alpha_t)
    X = np.random.normal(size=(S,d))
...
def calc_samples_accl_genli_old(dist0, alpha_t, S=10000):
    # Note: alpha_t[i] = \alpha_{i+1}, alpha_t_bar[i] = \bar{\alpha}_{i+1}
    assert(alpha_t[0] > 0)
    alpha_t_bar = np.cumprod(alpha_t)
    X = np.random.normal(size=(S,d))
    for t in range(len(alpha_t)-1,-1,-1):
        # print(t, end=" ")
        Z = np.random.normal(size=(S,d))
        W = np.random.normal(size=(S,d))
        V = W * np.sum(W * Z, axis=1, keepdims=True)
        score = dist0.score(X, alpha_t_bar[t])
        temp = score * np.sum(score * Z, axis=1, keepdims=True)
        X = (X + (1-alpha_t[t]) * score) / np.sqrt(alpha_t[t]) + np.sqrt((1-alpha_t[t]) / alpha_t[t]) * (Z - (1-alpha_t[t])/2/(1-alpha_t_bar[t]) * (Z + (1-alpha_t_bar[t]) * temp - V))
    # print("")
    return X
...
for i, n in enumerate(n_value):
    ...
    while True:
        ...
        X = calc_samples_accl_genli_old(dist0, alpha_t, Nkern)
        comp1[i] = time.time() - start_time
        print("Sec for genli old:", comp1[i])
        print("Estimating KL for n =", n)
        kl1[i] = calc_kl_from_mix_samp(dist0, X, Nkern)
        if kl1[i] == 0.:
            continue
...
np.save(FOLDER + "kl_mixture_genli_old.npy", kl1)
np.save(FOLDER + "kl_mixture_genli_old_comp.npy", comp1)
